...
// `requires` is an optional companion-token check (same idea as trigger-system's
// CONTEXT_REQUIRED) — without it, "wolf howl" fired on any "howled" token,
// including "the wind howled", since this classifier only checks token
// presence, not word order.
const EVENT_KEYWORDS = {
    'door knock':     { words: ['knock', 'knocked', 'knocking'] },
    'door slam':      { words: ['slam', 'slammed', 'slamming'] },
    'door creak':     { words: ['creak', 'creaked', 'creaking'] },
    'thunder':        { words: ['thunderclap', 'thundered'] },
    'scream':         { words: ['scream', 'screamed', 'shriek', 'shrieked'] },
    'glass break':    { words: ['shatter', 'shattered', 'shattering'] },
    'footsteps':      { words: ['footsteps', 'footstep'] },
    'wolf howl':      { words: ['howl', 'howled', 'howling'], requires: ['wolf', 'wolves'] },
    'metal crash':    { words: ['crash', 'crashed', 'crashing'] },
};
...
export function classifyLocal(transcript, opts = {}) {
    ...
    const sfx = [];
    for (const [id, event] of Object.entries(EVENT_KEYWORDS)) {
        const hasWord = event.words.some(w => tokens.includes(w));
        const hasCompanion = !event.requires || event.requires.some(w => tokens.includes(w));
        if (hasWord && hasCompanion) {
